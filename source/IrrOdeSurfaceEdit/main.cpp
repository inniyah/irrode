...
class myReceiver : public IEventReceiver {
  protected:
    ...
    void selectSet(u32 iIdx) {
      ...
      m_pApprox1_1->setChecked(p->getModeApprox1_1());
      m_pApprox1_2->setChecked(p->getModeApprox1_2());
    ...
    void updateGUI() {
      ...
      bool b=wcscmp(m_pEdVMaxLin->getText(),L"") && atof(stringc(m_pEdVMaxLin->getText()).c_str())!=0.0f && bParam;

      m_pStAddDampLin->setVisible(m_pCbDamps->isChecked() && b);
      m_pEdAddDampLin->setVisible(m_pCbDamps->isChecked() && b);

      b=wcscmp(m_pEdVMaxAng->getText(),L"") && atof(stringc(m_pEdVMaxAng->getText()).c_str())!=0.0f && bParam;
  ...
  public:
    myReceiver(IrrlichtDevice *pDevice) {
      ...
      m_pEdSlip2    =m_pGuienv->addEditBox(L"",rect<s32>(pos,dim),true,NULL,eEdSlip2    ); pos+=dist; pos+=dist; pos+=dist; pos+=dist;
      ...
      m_pApprox1_1=m_pGuienv->addCheckBox(true,rect<s32>(pos,cb)); pos+=dist;
      m_pApprox1_2=m_pGuienv->addCheckBox(true,rect<s32>(pos,cb)); pos+=dist;
      ...
      t=m_pGuienv->addStaticText(L"Approx1_1:  "   ,rect<s32>(pos,dim)); t->setDrawBackground(true); t->setTextAlignment(EGUIA_LOWERRIGHT,EGUIA_CENTER); pos+=dist;
      t=m_pGuienv->addStaticText(L"Approx1_2:  "   ,rect<s32>(pos,dim)); t->setDrawBackground(true); t->setTextAlignment(EGUIA_LOWERRIGHT,EGUIA_CENTER); pos+=dist;
    ...
    virtual bool OnEvent(const SEvent &event) {
      ...
      if (event.EventType==EET_GUI_EVENT) {
        if (event.GUIEvent.EventType==EGET_CHECKBOX_CHANGED && m_pActive!=NULL) {
          ...
          if (event.GUIEvent.Caller==m_pApprox1_1) { m_pActive->setModeApprox1_1(b); }
          if (event.GUIEvent.Caller==m_pApprox1_2) { m_pActive->setModeApprox1_2(b); }
          if (event.GUIEvent.Caller==m_pCbDamps  ) {
            ...
            bool b2=wcscmp(m_pEdVMaxLin->getText(),L"") && atof(stringc(m_pEdVMaxLin->getText()).c_str())!=0.0f;

            m_pStAddDampLin->setVisible(m_pCbDamps->isChecked() && b);
            m_pEdAddDampLin->setVisible(m_pCbDamps->isChecked() && b);

            b2=wcscmp(m_pEdVMaxAng->getText(),L"") && atof(stringc(m_pEdVMaxAng->getText()).c_str())!=0.0f;
        ...
        if (event.GUIEvent.EventType==EGET_ELEMENT_FOCUS_LOST && event.GUIEvent.Caller->getType()==EGUIET_EDIT_BOX && m_pActive!=NULL) {
          ...
          switch (event.GUIEvent.Caller->getID()) {
            ...
            case eVMaxLin:
              b=wcscmp(m_pEdVMaxLin->getText(),L"") && atof(stringc(m_pEdVMaxLin->getText()).c_str())!=0.0f;
            ...
            case eVMaxAng:
              b=wcscmp(m_pEdVMaxAng->getText(),L"") && atof(stringc(m_pEdVMaxAng->getText()).c_str())!=0.0f;
