...
void addNewJoint(ISceneManager *smgr, IVideoDriver *driver, CIrrOdeSceneNode *parent, vector3df pos, vector3df rot, ESCENE_NODE_TYPE iType, bool bLinkWorld, bool bMotor) {
  ...
  if (!bLinkWorld) {
    addNewBody(smgr,driver,pJoint,vector3df(0,-25,0),rot,vector3df(10,10,10),(ESCENE_NODE_TYPE)IRR_ODE_GEOM_SPHERE_ID,false);
  }
...
int main(int argc, char** argv) {
  ...
  char c=' ';
  do { std::cin >> c; printf("%c",c); } while (c!='y' && c!='n');
  bShadows=c=='y';
  ...
  //set IrrODE's timer
  ITimer *pTimer=CIrrOdeManager::getSharedInstance()->getTimer();
  ...
  CIrrOdeManager::getSharedInstance()->getQueue()->addEventListener(pColEvtListener);
  ...
  CIrrOdeManager::getSharedInstance()->getQueue()->addEventListener(rayListener);
  ...
  while(device->run()) {
    ...
    if (theEventReceiver.newHinge2Joint())
      addNewJoint(smgr,driver,worldNode,cam->getPosition(),cam->getRotation(),(ESCENE_NODE_TYPE)IRR_ODE_JOINT_HINGE2_ID     ,false                       ,theEventReceiver.ctrlDown());
